...
func save_game(slot_index: int = 1, custom_name: String = "") -> bool:
	...
	if game_manager and game_manager.current_player:
		save_data["player_position"] = {
			"x": game_manager.current_player.global_position.x,
			"y": game_manager.current_player.global_position.y
		}
...
func _create_save_data(save_name: String = "") -> Dictionary:
	...
	var save_data = {
		...
		"current_map": game_manager.current_map if game_manager else "",
	...
	if player_data_node and player_data_node.has_method("to_dictionary"):
		save_data["player_data"] = player_data_node.to_dictionary()
...
func get_save_info(slot_index: int) -> Dictionary:
	...
	info["timestamp"] = int(save_data.get("timestamp", 0))
